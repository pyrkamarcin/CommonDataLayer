...
mod tests {
    ...
    fn test_errors() -> Result<()> {
        snapshot_runner::test_snapshots("error", |input| {
            let view = input.get_json("view").expect("view");
            let tree_response = input.get_json("tree_response").expect("tree_response");
            let relation_map = input.get_json("relation_map").expect("relation_map");
            let objects: BTreeMap<ObjectIdPair, Value> =
            ...
            let result = (move || {
                let view_plan = ViewPlan::try_new(view, tree_response, relation_map)
                    .context("Could not build view plan")?;
                let mut buffer = ObjectBuffer::new(view_plan);
                let row_builder = RowBuilder::new();
                ...
                objects
                    ...
                    .flat_map(|row| row_builder.build(row).transpose())
                    .collect::<Result<Vec<_>>>()
                    .context("Could not build rows")
            })();

            let error = result.expect_err("build rows err");

            // Custom error display implementation to skip RUST_BACKTRACE
            let mut result = error.to_string();
            let caused: Vec<_> = error.chain().skip(1).collect();
            if caused.len() > 0 {
                result += "\n\nCaused by:";
                for c in caused {
                    result += &format!("\n    {}", c);
